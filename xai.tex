...
\newm{\begin{definition}[Minimal hitting set duality]
	...
	$S_2$ = MHS($S_1$), where $\mhs(S)$ indicates the minimal hitting sets of $S$,
	i.e. \newmm{$\mhs(S)$ is the minimal sets sharing at least one element with each subset in
		$S$.}
	% 
	\newmm{Concretely, given $\mins(S) = \{ s \in S ~|~ \forall t
\subsetneq s,~t \not\in S\}$ returning the subset-minimal elements of a
set of sets~$S$, and $\hs(S) = \{ t \subseteq (\cup S) ~|~ \forall s \in S,~
t \cap s \neq \emptyset\}$, $\mhs(S) = \mins(\hs(S))$.}
\end{definition}}
%
...
and vice versa: each CXp is \newmm{a} MHS of the set of all AXp's.}
